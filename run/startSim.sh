...
for i in `seq 1 ${NSIMWORKERS}`; do
  xterm -geometry 80x25+500+0 -e O2SimDeviceRunner --control static --id worker --mq-config ~/alisw_new/O2/run/primary-server.json &
done

# one hit merger -> the time measures the walltime of the complete session
time O2HitMergerRunner --id hitmerger --control static --mq-config ~/alisw_new/O2/run/primary-server.json 
